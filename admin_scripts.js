        ...
        if(data.success) {
          data.coupons.forEach(c => {
            ...
            tr.innerHTML = `
              ...
              <td data-label="Discount">${c.discountPercent}%</td>
    ...
    function applyFilters() {
      ...
      const filtered = allOrders.filter(o => {
        ...
        if (currentMenuTab === 'pendingPayment') {
           if (o.advancePaymentStatus !== 'পেন্ডিং' && o.advancePaymentStatus !== 'অপেক্ষমান' && o.advancePaymentStatus !== 'Pending') matchTab = false;
        } else if (currentMenuTab === 'ongoingOrder' || currentMenuTab === 'pendingDelivery') {
           const isDelivered = (o.status === 'ডেলিভারড' || o.status === 'Delivered');
           const isCancelled = (o.status === 'অর্ডার বাতিল' || o.status === 'বাতিল' || o.status === 'Cancelled' || o.advancePaymentStatus === 'পেমেন্ট বাতিল' || o.advancePaymentStatus === 'বাতিল' || o.advancePaymentStatus === 'Rejected');
           if (isDelivered || isCancelled) matchTab = false;
    ...
    function updateStats(all) {
      ...
      all.forEach(o => {
        if (o.advancePaymentStatus === 'পেন্ডিং' || o.advancePaymentStatus === 'অপেক্ষমান' || o.advancePaymentStatus === 'Pending') pendingPay++;
        ...
        if (o.advancePaymentStatus === 'কনফার্মড' || o.advancePaymentStatus === 'ভেরিফাইড' || o.advancePaymentStatus === 'Confirmed') {
          ...
          halfPay++;
        } else if (o.advancePaymentStatus === 'সম্পূর্ণ পরিশোধিত' || o.advancePaymentStatus === 'Fully Paid') {
          revenue += actualTotal;
        }
    ...
    function renderOrders(orders) {
      ...
      orders.forEach(o => {
        ...
        tr.innerHTML = `
          <td data-label="Order ID" style="font-family: monospace; color: var(--admin-primary);">${o.orderId || ''}</td>
          ...
          <td data-label="Payment Status"><span class="badge ${payBadge}">${payText}</span></td>
          <td data-label="Due Payment">
            ${o.duePaymentStatus ? `
              <div style="font-size:0.75rem; font-weight:bold; color:${o.duePaymentStatus==='পেন্ডিং'?'#ff9800':'#00d46a'};">${o.duePaymentStatus}</div>
              ${o.duePaymentPhone ? `<div style="font-size:0.7rem; color:var(--admin-text-sub);">Num: ${o.duePaymentPhone}</div>` : ''}
              ${o.duePaymentScreenshot ? `<div style="margin-top:2px;"><a href="#" onclick="openImageModal('${o.duePaymentScreenshot}', event)" style="font-size:0.7rem; color:#3b82f6;"><i class="fa-solid fa-image"></i> SS View</a></div>` : ''}
              ${o.duePaymentStatus==='পেন্ডিং' ? `<div style="margin-top:4px;"><button class="btn-action" style="background:#10b981; padding:2px 4px; font-size:0.7rem;" onclick="updateOrder(event, '${o.id}', 'payment', 'সম্পূর্ণ পরিশোধিত')">Verify Due</button></div>` : ''}
            ` : `<span style="color:var(--admin-text-sub); font-size:0.8rem;">-</span>`}
          </td>
          <td data-label="Order Status"><span class="badge ${statusBadge}">${statusText}</span></td>
          <td data-label="Details">
            ...
            ${o.paymentScreenshot ? `<div style="margin-top: 4px;"><a href="#" onclick="openImageModal('${o.paymentScreenshot}', event)" class="btn-action" style="background:#10b981; padding:4px 8px; font-size:0.75rem; text-decoration:none; display:inline-block; width:100%; box-sizing:border-box; text-align:center;"><i class="fa-solid fa-image"></i> Payment SS</a></div>` : ''}
          </td>
          <td data-label="Update Payment">
    ...
    function viewOrderDetails(id) {
      ...
      if (order.paymentScreenshot) {
         content += `<div style="margin-bottom:15px;"><strong>Payment Screenshot:</strong><br><div style="margin-top:5px;"><a href="#" onclick="openImageModal('${order.paymentScreenshot}', event)"><img src="${order.paymentScreenshot}" style="max-width: 100%; max-height: 300px; border-radius:6px; border:1px solid var(--admin-border);" alt="Payment Screenshot"></a></div></div>`;
      }
    ...
    function updateNotifications(orders) {
      ...
      let newCount = 0;
      orders.forEach(o => {
        if (o.advancePaymentStatus === 'পেন্ডিং' || o.advancePaymentStatus === 'অপেক্ষমান' || o.advancePaymentStatus === 'Pending') {
