...
const newSysInstruct = `"You are the official chat and voice assistant for LoveWeb, a platform for creating relationship and anniversary wishing websites. You must answer questions in Bengali (বাংলা).
...
code = code.replace(/const systemInstruction = ".*?";/, `const systemInstruction = ${newSysInstruct};`);
code = code.replace(/systemInstruction: "You are the official voice assistant.*?",/, `systemInstruction: ${newSysInstruct},`);
