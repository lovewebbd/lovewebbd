// Supabase Mock (Replaced for AI Studio compatibility)
const _supabase = (() => {
    const getDB = (table) => JSON.parse(localStorage.getItem('mock_db_' + table) || '[]');
    const saveDB = (table, data) => localStorage.setItem('mock_db_' + table, JSON.stringify(data));
    
    return {
        functions: {
            invoke: async () => ({ error: null, data: {} })
        },
        from: (table) => ({
            select: () => {
                let data = getDB(table);
                const builder = {
                    eq: (col, val) => { data = data.filter(r => r[col] === val); return builder; },
                    gte: (col, val) => { data = data.filter(r => r[col] >= val); return builder; },
                    or: (cond) => {
                        if (cond.includes('email.eq') && cond.includes('username.eq')) {
                            let id = cond.split('email.eq.')[1].split(',')[0];
                            data = data.filter(r => r.email === id || r.username === id);
                        }
                        return builder;
                    },
            ...
            delete: () => {
                const builder = {
                    eq: (col, val) => {
                        let db = getDB(table);
                        db = db.filter(r => r[col] !== val);
                        saveDB(table, db);
                        return Promise.resolve({ data: null, error: null });
                    },
                    in: (col, vals) => {
                        let db = getDB(table);
                        db = db.filter(r => !vals.includes(r[col]));
                        saveDB(table, db);
                        return Promise.resolve({ data: null, error: null });
                    }
                };
